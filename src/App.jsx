...

{
  ...
  "fugas": [
    {
      ...
      "importe_mensual": 0,
      "frecuencia": "mensual",
      "descripcion": "descripción de la fuga",
      "prioridad": "ALTA",
      "accion": "acción recomendada"
    }
  ],
  "resumen_categorias": [
    {"categoria": "CAT-1", "nombre": "Suscripciones sin uso", "fugas": 0, "impacto": 0}
  ],
  "proyeccion": {
    "mensual": 0,
    "anual": 0,
  ...
  "inconsistencias": ["texto de cada inconsistencia"],
  "proximos_pasos": ["paso 1", "paso 2", "paso 3", "paso 4"]
}

Tono: profesional, directo, sin alarmar. Sé específico. No inventes datos. Si un cargo es ambiguo, descríbelo como aparece y solicita verificación. Coloca ambiguos en CAT-8.`;
...
const catLabel = c => ({
  "CAT-1": "Suscripciones sin uso",
  "CAT-2": "Licencias duplicadas",
  "CAT-3": "Servicios olvidados",
  "CAT-4": "Proveedores inactivos",
  "CAT-5": "Comisiones bancarias",
  "CAT-6": "Seguros revisables",
  "CAT-7": "Infraestructura innecesaria",
  "CAT-8": "Sin justificación clara"
}[c] || c);

const fmt = n => "$" + Number(n || 0).toLocaleString("en-US", { minimumFractionDigits: 0, maximumFractionDigits: 0 });

const inputStyle = {
  width: "100%", padding: "10px 14px", background: "#1a1a1a", border: `1px solid ${BORDER}`,
...
const btnPrimary = {
  padding: "12px 32px", background: ACCENT, color: "#000", border: "none", borderRadius: 8,
...
Analiza ambas fuentes, cruza los datos y genera el informe JSON según las instrucciones.`;
  ...
  const downloadPDF = () => {
    ...
    const fugasHTML = (r.fugas || []).map(f2 => `
      <div class="fuga-card ${f2.prioridad.toLowerCase()}">
        <div class="fuga-header">
          <div class="fuga-nombre">${f2.nombre}</div>
          <div class="fuga-prio" style="color:${prioColor(f2.prioridad)}">${prioLabel(f2.prioridad)}</div>
        </div>
        <div class="fuga-meta">
          <span class="tag">${f2.categoria} — ${catLabel(f2.categoria)}</span>
          <span class="tag">${f(f2.importe_mensual)}/mes</span>
          <span class="tag">${f2.frecuencia}</span>
        </div>
        <p class="fuga-desc">${f2.descripcion}</p>
        <div class="fuga-accion">→ ${f2.accion}</div>
      </div>
    `).join("");

    const catRows = (r.resumen_categorias || []).filter(c => c.fugas > 0).map(c => `
      <tr><td>${c.categoria}</td><td>${c.nombre}</td><td class="center">${c.fugas}</td><td class="right red">${f(c.impacto)}</td></tr>
    `).join("");

    const inconsHTML = (r.inconsistencias || []).map(t => `<li>${t}</li>`).join("");
    const pasosHTML = (r.proximos_pasos || []).map((p, i) => `
      <div class="paso"><span class="paso-num">${i + 1}</span><span>${p}</span></div>
    `).join("");

    const html = `<!DOCTYPE html><html lang="es"><head><meta charset="UTF-8">
<title>Informe FugaZero IA — ${r.empresa || form.empresa}</title>
<style>
@import url('https://fonts.googleapis.com/css2?family=Inter:wght@400;500;600;700;800&display=swap');
*{margin:0;padding:0;box-sizing:border-box}
body{font-family:'Inter','Segoe UI',system-ui,sans-serif;color:#1a1a1a;background:#fff;font-size:13px;line-height:1.6}
.header{background:#0a0a0a;padding:32px 48px;display:flex;justify-content:space-between;align-items:center}
.logo{font-size:26px;font-weight:800;color:#fff}.logo span{color:#00C48C}
.tagline{font-size:11px;color:#888;margin-top:2px;letter-spacing:1px;text-transform:uppercase}
.header-right{text-align:right;color:#aaa;font-size:11px;line-height:1.8}
.header-right strong{color:#fff;font-size:13px}
.accent-bar{height:4px;background:linear-gradient(90deg,#00C48C,#00a374)}
.content{padding:36px 48px}
.section-title{font-size:11px;font-weight:700;color:#00C48C;text-transform:uppercase;letter-spacing:2px;margin-bottom:14px;margin-top:32px;padding-bottom:6px;border-bottom:1px solid #e5e7eb}
.resumen-grid{display:flex;gap:12px;margin-bottom:8px}
.resumen-card{flex:1;background:#f8f9fa;border:1px solid #e5e7eb;border-radius:8px;padding:16px;text-align:center}
.resumen-card.red .valor{color:#dc2626}.resumen-card.yellow .valor{color:#d97706}.resumen-card.green .valor{color:#059669}
.valor{font-size:24px;font-weight:800}.label{font-size:10px;color:#6b7280;margin-top:3px;font-weight:500;text-transform:uppercase;letter-spacing:.5px}
.fuga-card{border:1px solid #e5e7eb;border-radius:8px;padding:16px;margin-bottom:10px;border-left:4px solid #e5e7eb;page-break-inside:avoid}
.fuga-card.alta{border-left-color:#dc2626}.fuga-card.media{border-left-color:#d97706}.fuga-card.baja{border-left-color:#059669}
.fuga-header{display:flex;justify-content:space-between;align-items:center;margin-bottom:8px}
.fuga-nombre{font-size:14px;font-weight:700;color:#111}.fuga-prio{font-size:11px;font-weight:700}
.fuga-meta{display:flex;gap:6px;flex-wrap:wrap;margin-bottom:8px}
.tag{background:#f3f4f6;color:#374151;border-radius:4px;padding:2px 8px;font-size:11px;font-weight:500}
.fuga-desc{color:#374151;font-size:12px;margin-bottom:8px}.fuga-accion{font-size:12px;color:#059669;font-weight:600}
table{width:100%;border-collapse:collapse;margin-top:4px}
th{background:#f3f4f6;font-size:11px;font-weight:700;color:#374151;text-transform:uppercase;letter-spacing:.5px;padding:10px 12px;text-align:left}
td{padding:9px 12px;font-size:12px;border-bottom:1px solid #f3f4f6}
tr.total td{font-weight:700;background:#f8f9fa;border-top:2px solid #e5e7eb}
.center{text-align:center}.right{text-align:right}.red{color:#dc2626;font-weight:600}
.proy-grid{display:flex;gap:12px}
.proy-card{flex:1;background:#f0fdf7;border:1px solid #a7f3d0;border-radius:8px;padding:16px;text-align:center}
.proy-card .valor{font-size:22px;font-weight:800;color:#059669}
.proy-card .label{font-size:10px;color:#065f46;margin-top:3px;font-weight:600;text-transform:uppercase;letter-spacing:.5px}
.inconsistencias-list{list-style:none}
.inconsistencias-list li{padding:7px 0;border-bottom:1px solid #f3f4f6;font-size:12px;color:#374151}
.inconsistencias-list li::before{content:"⚠ ";color:#d97706}
.paso{display:flex;gap:12px;align-items:flex-start;padding:8px 0;border-bottom:1px solid #f3f4f6}
.paso-num{background:#00C48C;color:#fff;border-radius:50%;width:22px;height:22px;display:flex;align-items:center;justify-content:center;font-size:11px;font-weight:800;flex-shrink:0}
.legal{background:#f8f9fa;border:1px solid #e5e7eb;border-radius:6px;padding:14px 16px;margin-top:32px;font-size:10.5px;color:#6b7280;line-height:1.6}
.footer{background:#0a0a0a;padding:16px 48px;display:flex;justify-content:space-between;align-items:center;margin-top:32px}
.footer-left{color:#555;font-size:10px}.footer-right{color:#00C48C;font-size:11px;font-weight:700}
@media print{body{-webkit-print-color-adjust:exact;print-color-adjust:exact}.fuga-card{page-break-inside:avoid}}
</style></head><body>
<div class="header">
  <div><div class="logo">Fuga<span>Zero</span> IA</div><div class="tagline">Auditoría de Fugas Financieras</div></div>
  <div class="header-right"><strong>${r.empresa || form.empresa}</strong><br>Período: ${r.periodo || "—"}<br>Informe: ${today}</div>
</div>
<div class="accent-bar"></div>
<div class="content">
  <div class="section-title">Resumen Ejecutivo</div>
  <div class="resumen-grid">
    <div class="resumen-card red"><div class="valor">${r.resumen?.total_fugas || 0}</div><div class="label">Fugas detectadas</div></div>
    <div class="resumen-card yellow"><div class="valor">${f(r.resumen?.total_mensual)}</div><div class="label">Impacto mensual</div></div>
    <div class="resumen-card green"><div class="valor">${f(r.resumen?.total_anual)}</div><div class="label">Impacto anual</div></div>
  </div>
  <p style="font-size:12px;color:#4b5563;margin-top:12px;">En el período analizado se detectaron <strong>${r.resumen?.total_fugas || 0} fugas financieras activas</strong> por un total de <strong>${f(r.resumen?.total_mensual)}/mes</strong> (${f(r.resumen?.total_anual)}/año).</p>
  <div class="section-title">Fugas Detectadas</div>
  ${fugasHTML || "<p style='color:#6b7280;font-size:12px;'>No se detectaron fugas.</p>"}
  <div class="section-title">Resumen por Categoría</div>
  <table><thead><tr><th>Código</th><th>Categoría</th><th class="center">Fugas</th><th class="right">Impacto/mes</th></tr></thead>
  <tbody>${catRows}<tr class="total"><td colspan="2">TOTAL</td><td class="center">${r.resumen?.total_fugas || 0}</td><td class="right red">${f(r.resumen?.total_mensual)}</td></tr></tbody></table>
  <div class="section-title">Proyección de Ahorro</div>
  <div class="proy-grid">
    <div class="proy-card"><div class="valor">${f(r.proyeccion?.mensual)}</div><div class="label">Mensual</div></div>
    <div class="proy-card"><div class="valor">${f(r.proyeccion?.anual)}</div><div class="label">Anual</div></div>
    <div class="proy-card"><div class="valor">${f(r.proyeccion?.tres_anos)}</div><div class="label">3 años</div></div>
  </div>
  ${r.inconsistencias?.length ? `<div class="section-title">Inconsistencias Detectadas</div><ul class="inconsistencias-list">${inconsHTML}</ul>` : ""}
  <div class="section-title">Próximos Pasos Recomendados</div>
  ${pasosHTML}
  <div class="legal"><strong>Nota legal:</strong> Este informe ha sido generado mediante análisis automatizado asistido por inteligencia artificial. Las cifras son estimaciones basadas en los datos disponibles. Se recomienda verificar cada hallazgo antes de tomar decisiones financieras. FugaZero IA no se responsabiliza de decisiones tomadas sin la debida diligencia profesional.</div>
</div>
<div class="footer">
  <div class="footer-left">kaelenai.com/fugazero · fugazero@kaelenai.com</div>
  <div class="footer-right">FugaZero IA · ${today}</div>
</div>
<script>window.onload=()=>window.print()</script>
  ...
  if (step === 0) return (
    <div style={{ minHeight:"100%", background:BG, color:TEXT, fontFamily:"'Inter','Segoe UI',system-ui,sans-serif", display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center", padding:"40px 20px", textAlign:"center" }}>
      ...
      <div style={{ color:MUTED, fontSize:16, maxWidth:440, lineHeight:1.6, marginBottom:36 }}>Detecta gastos recurrentes invisibles que drenan la liquidez de tu empresa mes a mes. Sube tu extracto bancario y recibe un informe profesional en minutos.</div>
    ...
    <div style={{ minHeight:"100%", background:BG, color:TEXT, fontFamily:"'Inter','Segoe UI',system-ui,sans-serif", padding:"32px 20px", maxWidth:540, margin:"0 auto" }}>
      ...
      <div style={{ display:"flex", flexDirection:"column", gap:22 }}>
        <div>
          ...
          <input style={inputStyle} value={form.empresa} onChange={e => setForm({...form, empresa:e.target.value})} placeholder="Ej: Logística Andina SRL" />
        </div>
        <div>
          <label style={labelStyle}>Sector / Industria</label>
          <input style={inputStyle} value={form.sector} onChange={e => setForm({...form, sector:e.target.value})} placeholder="Ej: Transporte y logística" />
        </div>
        <div>
          <label style={labelStyle}>Tamaño del negocio</label>
          <input style={inputStyle} value={form.tamano} onChange={e => setForm({...form, tamano:e.target.value})} placeholder="Ej: 12 empleados, $25K/mes de facturación" />
          <div style={{ fontSize:12, color:MUTED, marginTop:5, lineHeight:1.5 }}>Este dato permite a la IA detectar si ciertos servicios están sobredimensionados para tu negocio.</div>
        </div>
        <div>
          ...
          <textarea style={{...inputStyle, minHeight:70, resize:"vertical"}} value={form.herramientas} onChange={e => setForm({...form, herramientas:e.target.value})} placeholder="Ej: Gmail, Zoom, QuickBooks, Slack, Canva, ChatGPT..." />
          <div style={{ fontSize:12, color:MUTED, marginTop:5, lineHeight:1.5 }}>Escribe las que recuerdes. La IA marcará los cargos que no menciones aquí como posibles fugas.</div>
        </div>
        <div>
          <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:6 }}>
          ...
          <textarea style={{...inputStyle, minHeight:70, resize:"vertical"}} value={form.proveedores} onChange={e => setForm({...form, proveedores:e.target.value})} placeholder="Ej: contador externo, agencia de diseño, empresa de limpieza..." />
          <div style={{ fontSize:12, color:MUTED, marginTop:5, lineHeight:1.5 }}>Si no estás seguro, déjalo en blanco y la IA lo detectará en el extracto.</div>
        </div>
        <div>
          <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:6 }}>
          ...
          <div style={{ fontSize:12, color:MUTED, marginTop:5, lineHeight:1.5 }}>Incluye suscripciones, licencias y servicios contratados. No incluyas sueldos, impuestos ni servicios básicos.</div>
        </div>
      </div>
      ...
      <div style={{ minHeight:"100%", background:BG, color:TEXT, fontFamily:"'Inter','Segoe UI',system-ui,sans-serif", padding:"28px 16px", maxWidth:640, margin:"0 auto" }}>
        <div ref={reportRef}>
          ...
          <div style={{ fontSize:11, color:MUTED, lineHeight:1.5, padding:"12px 0", borderTop:`1px solid ${BORDER}20` }}>
            Este informe ha sido generado mediante análisis automatizado asistido por inteligencia artificial. Las cifras son estimaciones. Se recomienda verificar cada hallazgo antes de tomar decisiones financieras.
          </div>
        </div>
